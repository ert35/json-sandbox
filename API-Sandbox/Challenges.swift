...
internal func exerciseOne() {
    ...
    let address = userData["results"][0]["location"]["street"].stringValue
    let cityUser = userData["results"][0]["location"]["city"].stringValue
    let stateUser = userData["results"][0]["location"]["state"].stringValue
    let postalCode = userData["results"][0]["location"]["postcode"].stringValue
    let userTitle = userData["results"][0]["name"]["title"].stringValue
    let email = userData["results"][0]["email"].stringValue
    let cellNum = userData["results"][0]["phone"].stringValue
    
    
    print("\(firstName) \(lastName) lives at \(address) in \(cityUser), \(stateUser), \(postalCode). If you want to contact \(userTitle). \(lastName), you can email \(email) or call at \(cellNum).")
...
internal func exerciseThree() {
     ...
     */
    var allMovies: [Movie] = []
    for n in allMoviesData{
         allMovies.append(Movie(json: n))
    }
    ...
    var count = 0
    for n in allMovies{
        if(n.rightsOwner.contains("Disney")){
            count += 1
            print(n.name)
            if (count  == 2){
                break;
    ...
    for n in allMovies{
        if n.releaseDate.contains("2016"){
            print("\(n.name) released on \(n.releaseDate)")
        }
    }
